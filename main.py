...
from telegram import send_async_telegram_message
    ...
    for coin in volume:
        ...
        if coin not in coins_bought:
            ...
            # error handling here in case position cannot be placed
                ...
                send_async_telegram_message(f"FAILED buying {volume[coin]} {coin} - {last_price[coin]['price']}")
            ...
            # run the else block if the position has been placed and return order info
                ...
                while orders[coin] == []:
                    ...
                    if LOG_TRADES:
                        write_log(f"Buy : {volume[coin]} {coin} - {last_price[coin]['price']}")
                    send_async_telegram_message(f"Buy : {volume[coin]} {coin} - {last_price[coin]['price']}")
...
def sell_coins():
    ...
    for coin in list(coins_bought):
        ...
        if LastPrice < SL or LastPrice > TP and not USE_TRAILING_STOP_LOSS:
            ...
            try:
                print(f"Selling {coin} amount {coins_bought[coin]['volume']}")
                if not TEST_MODE:
                    ...
                    print(f"Response: {sell_coins_limit}")

            # error handling here in case position cannot be placed
            except Exception as e:
                print(f"Selling exception {e}")
                send_async_telegram_message(f"FAILED selling {coin} amount {coins_bought[coin]['volume']}")
                try:
                    last_symbol = int(str(coins_bought[coin]['volume'])[-1])
                    quantity_test_str = str(coins_bought[coin]['volume'])[:-1]
                    quantity_test = quantity_test_str + str(last_symbol-1)
                    print(f"Selling {coin} amount {quantity_test}")
                    sell_coins_limit = client.create_order(
                        symbol=coin,
                        side='SELL',
                        type='MARKET',
                    ...
                    print(f"Selling with lower price exception {e}")
            ...
            # run the else block if coin has been sold and create a dict for each coin sold
                ...
                send_async_telegram_message(f"Sell: {coins_sold[coin]['volume']} {coin} - {BuyPrice} - {LastPrice} Profit: {profit:.2f} {PriceChange - (TRADING_FEE * 2):.2f}%")
            continue
